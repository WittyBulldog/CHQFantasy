...
if (typeof document !== 'undefined') {
  ...
  const leadingWins=result.rankings[0]?.wins ?? 0;
  for(const r of result.rankings.filter(r=>r.wins>0)){
    const tied=result.rankings.filter(other=>other.rank===r.rank).length>1;
    const rank=leadingWins ? `${tied?'T':'#'}${r.rank}` : '—';
    const item=add(list,'li','');
    if(leadingWins && r.rank===1)item.className='award-leader';
    add(item,'h4',`${rank} ${r.name}`);
    const stats=add(item,'dl','','award-row-stats');
    for(const [label,value] of [
      ['Weekly wins',String(r.wins)],
      ['Wins behind',String(leadingWins-r.wins)],
      ['Winning-week avg.',r.average===null?'—':r.average.toFixed(2)]
    ]){
      const cell=add(stats,'div','');add(cell,'dt',label);add(cell,'dd',value);
    }
    add(item,'p',r.weeks.length?`Winning weeks: ${r.weeks.join(', ')}`:'No weekly wins yet.');
  }
