...
const output = `<section class="swami-section" aria-labelledby="swami-heading">
  ...
  ${weeks.map(w => `<h3 class="swami-week">Week ${w.week} predictions &amp; results</h3>${w.summary ? `<p class="swami-note">${escape(w.summary)}</p><p class="swami-note">Verified ${escape(w.verified)}; subject to Yahoo stat corrections.</p>` : ''}<div class="swami-grid">${w.picks.map(p => `<article class="matchup-panel"><span class="swami-status">${labels[result(p)]}</span><h4>${p.teams.map(escape).join(' vs. ')}</h4><p class="swami-winner">Swami sez: ${escape(p.pick)}</p><p>${escape(p.comment)}</p>${p.winner !== null ? `<p><strong>Final result: ${escape(p.winner)}</strong>${p.scores ? `<br>${escape(p.teams[0])} ${p.scores[0].toFixed(2)} • ${escape(p.teams[1])} ${p.scores[1].toFixed(2)}` : ''}</p>${p.recap ? `<p>${escape(p.recap)}</p>` : ''}${p.source ? `<p><a href="${escape(p.source)}">View final matchup on Yahoo →</a></p>` : ''}` : ''}</article>`).join('\n')}</div>`).join('\n')}
</section>`;
